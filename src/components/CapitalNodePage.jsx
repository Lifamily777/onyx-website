...
<section className={styles.askSammi}><span><LocaleLabel value={"ASK SAMMI"} /></span><h2><Localized en={<>Bring the context, not sensitive documents.</>} zh={<>请提供大概背景信息，请不要发送敏感文件。</>} /></h2><Link to={`${localePath('/contact')}?context=${encodeURIComponent(node.askSammiContext)}`}><Localized en={<>Ask Sammi</>} zh={<>联系Sammi</>} /></Link></section><footer className={styles.disclaimer}><p><Localized en={<><LocaleLabel value={EDUCATIONAL_DISCLAIMER.en} /></>} zh={<><LocaleLabel value={EDUCATIONAL_DISCLAIMER.zh} /></>} /></p></footer></main>;
}
