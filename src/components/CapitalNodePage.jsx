...
function ToolResult({ result, nodeId }) {
  ...
  if (nodeId === 'W25') return <div className={styles.nextDollarResult}><header><strong>SEE · Compare six capital jobs</strong><span lang="zh-CN">查看 · 比较六种资本任务</span><p>Blank inputs are treated as zero. These cards stay in a fixed order: there is no ranking, winner, or universal best answer.</p><p lang="zh-CN">空白输入按零处理。卡片顺序固定，不提供排名、赢家或普遍最佳答案。</p></header><dl className={styles.assumptions}><div><dt>Reserve months</dt><dd>{result.assumptions.reserveMonths}</dd></div><div><dt>Highest APR</dt><dd>{result.assumptions.debtApr}%</dd></div><div><dt>Known expense gap</dt><dd>${result.assumptions.knownExpenseGap.toLocaleString('en-US')}</dd></div><div><dt>Tax reserve need</dt><dd>${result.assumptions.taxReserveNeed.toLocaleString('en-US')}</dd></div><div><dt>Investment horizon</dt><dd>{result.assumptions.investmentHorizon} years</dd></div><div><dt>Risk-capacity input</dt><dd>{result.assumptions.riskCapacity}/5</dd></div><div><dt>Available amount</dt><dd>${result.assumptions.availableAmount.toLocaleString('en-US')}</dd></div></dl><div className={styles.nextDollarGrid}>{result.jobs.map((job)=><article key={job.id}><h3>{job.label}<small lang="zh-CN">{job.labelZh}</small></h3><p>{job.role}</p><p lang="zh-CN">{job.roleZh}</p><div><strong>Current signal · 当前信号</strong><p>{job.signal}</p><p lang="zh-CN">{job.signalZh}</p></div><div><strong>Trade-off · 取舍</strong><p>{job.tradeoff}</p><p lang="zh-CN">{job.tradeoffZh}</p></div></article>)}</div></div>
  return <div className={styles.result}><strong>Educational result · 教育性结果</strong><pre>{JSON.stringify(result,null,2)}</pre></div>
}
