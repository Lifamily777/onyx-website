...
const stateCopy = {
  desired:{en:'Desired job',zh:'希望承担'},represented:{en:'Evidence observed',zh:'已有相关依据'},needs_review:{en:'Needs review',zh:'需要梳理'},unclear:{en:'Needs clarification',zh:'需要确认'},not_selected:{en:'Not selected',zh:'未选择'},
}
...
function ReviewResult({result,local,zh,localePath,onRestart}) {
  ...
  return <main className={`${styles.page} page-enter`}><div className={styles.results}>
    ...
    <ResultSection eyebrow="02 · CAPITAL JOB MAP" title={zh?'愿望、现状与下一块钱，分开来看':'Separate desired jobs, current coverage, and the next dollar'}><p className={styles.principle}>{local(result.capitalJobs.principle)}</p><div className={styles.jobColumns}><JobList title={zh?'希望资金承担':'DESIRED JOBS'} jobs={result.capitalJobs.desiredJobs} local={local}/><JobList title={zh?'现有资源依据':'CURRENT COVERAGE'} jobs={result.capitalJobs.existingMoney} local={local}/><JobList title={zh?'下一块钱':'NEXT DOLLAR'} jobs={result.capitalJobs.nextDollar} local={local}/></div></ResultSection>
