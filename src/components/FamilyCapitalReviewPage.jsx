...
const stateCopy = {
  desired:{en:'Desired job',zh:'希望承担'},represented:{en:'Related facts reported',zh:'已报告相关情况'},needs_review:{en:'Needs review',zh:'需要梳理'},unclear:{en:'Needs clarification',zh:'需要确认'},not_selected:{en:'Not selected',zh:'未选择'},
}
...
export default function FamilyCapitalReviewPage() {
  ...
  if (phase === 'intro') return <main className={`${styles.page} page-enter`}><section className={styles.intro}>
    ...
    <div className={styles.promiseGrid}><article><b>01</b><strong>{zh?'Foundation Map':'Foundation Map'}</strong><span>{zh?'看清基础与规划窗口':'See foundations and planning windows'}</span></article><article><b>02</b><strong>Capital Job Map</strong><span>{zh?'分开看愿望、现有资源与下一块钱':'Separate desired jobs, current resources, and the next dollar'}</span></article><article><b>03</b><strong>Action Map</strong><span>{zh?'把决定放进正确的时间顺序':'Put decisions in the right time horizon'}</span></article></div>
...
function ReviewResult({result,local,zh,localePath,onRestart}) {
  ...
  return <main className={`${styles.page} page-enter`}><div className={styles.results}>
    <header className={styles.resultHero}><p className={styles.eyebrow}>{zh?'家庭资本梳理结果':'FAMILY CAPITAL REVIEW'}</p><h1>{zh?'先看全局，再决定下一步。':'See the whole picture. Then decide what comes next.'}</h1><p>{zh?'这是一项初步筛查与决策梳理，不是分数或产品建议，也不判断税务或法律合规、退休资金或保险保障是否充足、文件法律效力、投资或产品是否适合。每项提示只反映已报告的信息和仍需确认的问题。':'This is a screening and decision-intelligence review—not a score or product recommendation. It does not determine tax or legal compliance, retirement or insurance adequacy, estate-document validity, investment suitability, or product suitability. Each signal reflects only what was reported and what still needs clarification.'}</p><div><span>{zh?'你最关心':'YOUR PRIORITY'}</span><strong>{local(result.clientPriority.label)}</strong></div>{result.onyxAlsoNoticed&&<div><span>{zh?'黑曜财商同时注意到':'ONYX ALSO NOTICED'}</span><strong>{local(result.onyxAlsoNoticed.title)}</strong></div>}</header>
    ...
    <ResultSection eyebrow="02 · CAPITAL JOB MAP" title={zh?'愿望、现有资源与下一块钱，分开来看':'Separate desired jobs, current resources, and the next dollar'}><p className={styles.principle}>{local(result.capitalJobs.principle)}</p><div className={styles.jobColumns}><JobList title={zh?'希望资金承担':'DESIRED JOBS'} jobs={result.capitalJobs.desiredJobs} local={local}/><JobList title={zh?'已报告的现有资源':'REPORTED CURRENT RESOURCES'} jobs={result.capitalJobs.existingMoney} local={local}/><JobList title={zh?'下一块钱':'NEXT DOLLAR'} jobs={result.capitalJobs.nextDollar} local={local}/></div></ResultSection>

    <ResultSection eyebrow="03 · ACTION MAP" title={zh?'先讨论决定，再讨论工具':'Prioritize decisions before tools'}><div className={styles.actionGrid}>{actionSections.map((section)=><article key={section.key}><h3>{section.title}</h3>{section.items.length?section.items.map((item)=><Link key={item.foundationId} to={localePath(item.route)}><strong>{local(item.title)}</strong><span>{local(item.action)}</span></Link>):<p>{zh?'目前没有归入这一时段的事项。':'No item currently falls in this horizon.'}</p>}</article>)}</div></ResultSection>

    <ResultSection eyebrow={zh?'04 · 决策线索':'04 · DECISION INTELLIGENCE'} title={zh?'重要的不是标签，而是接下来要弄清什么':'The useful question is what to clarify next'}><div className={styles.intelligence}>{result.decisionIntelligence.map((item)=><article key={item.id}><h3>{local(item.title)}</h3><p><b>{zh?'为什么重要：':'WHY IT MATTERS: '}</b>{local(item.whyFlagged)}</p><p><b>{zh?'需要决定：':'DECISION TO CLARIFY: '}</b>{local(item.decisionToClarify)}</p><p><b>{zh?'还需要：':'STILL NEEDED: '}</b>{local(item.informationToGather)}</p><p><b>{zh?'需要权衡：':'TRADE-OFF: '}</b>{local(item.tradeoffToConsider)}</p></article>)}</div></ResultSection>

    <section className={styles.sammi}><p className={styles.eyebrow}>{zh?'05 · 会谈摘要':'05 · CONTEXTUAL SAMMI REVIEW'}</p><h2>{zh?'把地图带进一次有重点的谈话。':'Bring the map into a focused conversation.'}</h2><div className={styles.sammiGrid}><SummaryList title={zh?'未发现即时问题':'NO IMMEDIATE SIGNAL'} items={result.sammiReview.healthy} local={local}/><SummaryList title={zh?'需要关注':'NEEDS ATTENTION'} items={result.sammiReview.needsAttention} local={local}/><SummaryList title={zh?'仍缺信息':'WHAT IS STILL UNKNOWN'} items={result.sammiReview.missingInformation} local={local}/><SummaryList title={zh?'需要梳理的资金任务':'UNDER-SERVED CAPITAL JOBS'} items={result.sammiReview.underservedCapitalJobs} local={local}/><SummaryList title={zh?'临近决定':'DECISIONS APPROACHING'} items={result.sammiReview.decisionsApproaching} local={local}/><QuestionList title={zh?'Sammi可以接着问':'WHAT SAMMI SHOULD ASK NEXT'} items={result.sammiReview.followUpQuestions} local={local}/></div><Link className={styles.primary} to={localePath('/contact')}>{zh?'联系 Sammi 讨论梳理结果':'Discuss This Review with Sammi'}</Link><p>{local(result.sammiConversationContext.notice)}</p><p>{zh?'最终判断仍需要客户事实、专业分析，以及适用情况下的持牌或法律专业审核。':'Final decisions still depend on client facts, professional analysis, and licensed or legal review where applicable.'}</p></section>
...
function SummaryList({title,items,local}) { return <article><h3>{title}</h3>{items.length?<ul>{items.map((item)=><li key={item.foundationId || item.id}>{local(item.title || item.label)}</li>)}</ul>:<p>—</p>}</article> }
function QuestionList({title,items,local}) { return <article><h3>{title}</h3>{items.length?<ul>{items.map((item)=><li key={item.id}>{local(item.question)}</li>)}</ul>:<p>—</p>}</article> }
