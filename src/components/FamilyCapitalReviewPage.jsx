...
function ReviewResult({result,local,zh,localePath,onRestart}) {
  ...
  return <main className={`${styles.page} page-enter`}><div className={styles.results}>
    <header className={styles.resultHero}><p className={styles.eyebrow}>{zh?'家庭资本梳理结果':'FAMILY CAPITAL REVIEW'}</p><h1>{zh?'先看全局，再决定下一步。':'See the whole picture. Then decide what comes next.'}</h1><p>{zh?'这是一项初步筛查与决策梳理，不是分数或产品建议，也不判断税务或法律合规、退休资金或保险保障是否充足、文件法律效力、投资或产品是否适合。每项提示只反映已报告的信息和仍需确认的问题。':'This is a screening and decision-intelligence review—not a score or product recommendation. It does not determine tax or legal compliance, retirement or insurance adequacy, estate-document validity, investment suitability, or product suitability. Each signal reflects only what was reported and what still needs clarification.'}</p><div><span>{zh?'你最关心':'YOUR PRIORITY'}</span><strong>{local(result.clientPriority.label)}</strong></div>{result.onyxAlsoNoticed&&<div><span>{zh?'黑曜同时注意到':'ONYX ALSO NOTICED'}</span><strong>{local(result.onyxAlsoNoticed.title)}</strong></div>}</header>
